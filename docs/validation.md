...
| Arcade keyboard drift | steering-only cannot arm Slide; a physical handbrake entry produces bounded readable sideslip, sustained steering-only countersteer, left/right symmetry and recovery; excessive handbrake remains capable of a spin. Chromium key events additionally verify the controlled outcome, unchanged throttle/brake/handbrake commands, finite tire telemetry, full-window Canvas and visible Overlay diagnostics. |
